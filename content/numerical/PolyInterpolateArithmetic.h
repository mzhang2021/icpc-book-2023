 ...
 * Time: $O(n + \log MOD)$
 * Status: AC on https://codeforces.com/contest/622/problem/F
 */

M interpolate(const vector<M> &y, long long x) {
	int n = (int) y.size(); if (x < n) return y[x];
	vector<M> pref(n + 1), suff(n + 1), inv(n + 1);
	M fact = pref[0] = suff[n] = 1;
	for (int i=0; i<n; i++) {
		pref[i+1] = pref[i] * (x - i); fact *= i + 1; }
	inv[n] = inverse(fact);
	for (int i=n-1; i>=0; i--) {
		suff[i] = suff[i+1] * (x - i); inv[i] = inv[i+1] * (i + 1); }
	M ret = 0;
	for (int i=0; i<n; i++) ret += (i % 2 == n % 2 ? -1 : 1) * y[i] * pref[i] * suff[i+1] * inv[i] * inv[n-i-1];
	return ret;
}
