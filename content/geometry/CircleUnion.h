/**
 * Author: YouKn0wWho
 * Source: https://github.com/ShahjalalShohag/code-library/blob/master/Geometry/Geometry%202D.cpp#L434
 * Description: Finds the area of union of circles.
 * Time: $O(n^2 \log n)$
 */
...
struct CircleUnion {
	...
	inline double angle(double A, double B, double C) {
		...
		if (val < -1) val = -1; if (val > +1) val = +1; return acos(val);
	}
	CircleUnion() { init(); }
