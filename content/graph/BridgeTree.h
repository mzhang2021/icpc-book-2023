 ...
 * Time: O(n + m)
 * Status: AC on https://codeforces.com/gym/100676 (problem H), https://www.hackerrank.com/contests/101hack26/challenges/sherlock-and-queries-on-the-graph/problem
 */

struct BridgeTree {
	int n, eid, ti; vector<int> num, id, stk;
	vector<vector<int>> tree, comp;
	vector<vector<pair<int, int>>> adj;
	BridgeTree(int _n) : n(_n), eid(0), ti(0), num(n), id(n), adj(n) {}
	void addEdge(int u, int v) {
		adj[u].emplace_back(v, eid);
		adj[v].emplace_back(u, eid); eid++;
	}
	void init() {
		for (int u=0; u<n; u++) if (!num[u]) {
			dfs(u, -1); comp.emplace_back();
			while (!stk.empty()) {
		...
		for (auto &c : comp) for (int u : c)
			for (auto [v, i] : adj[u]) if (id[u] != id[v])
				tree[id[u]].push_back(id[v]);
	}
	int dfs(int u, int p) {
		int low = num[u] = ++ti; stk.push_back(u);
