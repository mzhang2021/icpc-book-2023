 ...
 * Time: $O((n + m) \log n)$
 * Status: AC on https://cses.fi/problemset/task/1703
 */

struct DominatorTree {
	int ti; vector<int> label, rlabel, dom, sdom, par, best;
	...
	void init(int r = 0) {
		...
		for (int u=ti-1; u>=0; u--) {
			for (int v : radj[u]) sdom[u] = min(sdom[u], sdom[find(v)]);
			if (u > 0) bucket[sdom[u]].push_back(u);
			for (int v : bucket[u]) {
				int w = find(v); dom[v] = sdom[v] == sdom[w] ? sdom[v] : w; }
			for (int v : children[u]) par[v] = u;
		}
		for (int u=1; u<ti; u++) {
	...
	void dfs(int u) {
		label[u] = ti; rlabel[ti] = u;
		sdom[ti] = par[ti] = best[ti] = ti; ti++;
		for (int v : adj[u]) {
			if (label[v] == -1) {
				dfs(v); children[label[u]].push_back(label[v]); }
			radj[label[v]].push_back(label[u]);
		}
	}
	int find(int u) {
		if (par[u] != u) {
			int v = find(par[u]); par[u] = par[par[u]];
			if (sdom[v] < sdom[best[u]]) best[u] = v;
		}
		return best[u];
	}
};
