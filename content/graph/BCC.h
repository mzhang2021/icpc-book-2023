 ...
 * Time: O(n + m)
 * Status: AC on https://codeforces.com/contest/118/problem/E, https://cses.fi/problemset/task/1705/
 */

struct BCC {
	int n, ti; vector<int> num, id, stk; vector<bool> art;
	vector<pair<int, int>> edges; vector<vector<int>> tree, ecomp, vcomp; vector<vector<pair<int, int>>> adj;
	...
	void init(bool buildTree = false) {
		...
		if (buildTree) {
			...
			for (int u=0; u<n; u++) if (art[u]) {
				id[u] = (int) tree.size(); tree.emplace_back(); }
			for (int i=0; i<(int)vcomp.size(); i++) for (int u : vcomp[i]) {
				if (art[u]) { tree[id[u]].push_back(i); tree[i].push_back(id[u]); }
				else id[u] = i;
			}
		}
	}
	int dfs(int u, int p) {
		...
		for (auto [v, i] : adj[u]) if (i != p) {
			if (!num[v]) {
				stk.push_back(i);
				int ret = dfs(v, i); low = min(low, ret);
				if (num[u] <= ret) {
				...
				low = min(low, num[v]); stk.push_back(i); }
		}
		return low;
	}
};
